...
function AllPassed() {
    ...
    document.getElementById('my-name').innerText = client_id;
    console.log("Sending sender_id: ", client_id);  // 디버깅용 로그

    // 서버로 모든 테스트 케이스 통과 메시지를 보내는 fetch 요청
fetch("http://127.0.0.1:5000/all-tests-passed", {
    method: "POST",
    headers: {
        "Content-Type": "application/json"  // JSON 형식으로 전송하는 것을 명시
    },
    body: JSON.stringify({
        message: "All test cases passed", 
        sender_id: client_id  // 자신의 ID를 전송
    })
})
