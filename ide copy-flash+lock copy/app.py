...
submission_count = {}  # 클라이언트 ID와 제출 횟수를 저장
...
def handle_use_skill(data):
    ...
    if opponent_id:
        ...
        elif skill_type == 'decay':
            duration = 20
            socketio.emit('skill_effect', {'skill': 'decay', 'status': 'active'}, room=clients[opponent_id])
        
        elif skill_type == 'toxic_cloud':
            duration = 20
            socketio.emit('skill_effect', {'skill': 'toxic_cloud', 'status': 'active'}, room=clients[opponent_id])
...
def find_opponent(sender_id):
    for client_id in clients:
        if client_id != sender_id:
            # Emit the opponent's ID to both players
            socketio.emit('opponent_found', {'opponent_id': client_id}, room=clients[sender_id])
            socketio.emit('opponent_found', {'opponent_id': sender_id}, room=clients[client_id])
            return client_id
    return None

@socketio.on('character_count')
def handle_character_count(data):
    sender_id = data['sender_id']
    character_count = data['character_count']
    
    opponent_id = find_opponent(sender_id)
    if opponent_id:
        socketio.emit('opponent_character_count', {
            'opponent_id': sender_id,
            'character_count': character_count
        }, room=clients[opponent_id])
...
def handle_code_submission(data):
    ...
    # Initialize submission count for the sender if it doesn't exist
    if sender_id not in submission_count:
        submission_count[sender_id] = 0

    # Increment the submission count
    submission_count[sender_id] += 1

    if submission_count[sender_id] >= 5:
        # Mark sender as lost after 5 submissions
        states[sender_id] = 'lose'
        states[opponent_id] = 'won'

        # Notify both players of the result
        socketio.emit('test_result_notification', {'message': 'You Lose!'}, room=clients[sender_id])
        socketio.emit('test_result_notification', {'message': 'You Won!'}, room=clients[opponent_id])

    else:
        # Notify the sender's opponent that the sender has submitted code
        emit('submission_log', {
            'sender': sender_id,
            'submission_count': submission_count[sender_id]
        }, room=clients[opponent_id])  # Notify the opponent
